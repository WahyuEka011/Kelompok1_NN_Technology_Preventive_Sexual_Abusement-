...
void messageHandler(char* topic, byte* payload, unsigned int length) {
  ...
  if (message == "LED_BLUE_ON") {
    digitalWrite(LED_PIN_BLUE, HIGH);
  } else if (message == "LED_BLUE_OFF") {
    digitalWrite(LED_PIN_BLUE, LOW);
  } else if (message == "BUZZER_ON") {
    digitalWrite(BUZZER_PIN, HIGH);
  } else if (message == "BUZZER_OFF") {
    digitalWrite(BUZZER_PIN, LOW);
  } else if (message == "LED_GREEN_ON") {
    digitalWrite(LED_PIN_GREEN, HIGH);
  } else if (message == "LED_GREEN_OFF") {
    digitalWrite(LED_PIN_GREEN, LOW);
  } else if (message == "LED_YELLOW_ON") {
    digitalWrite(LED_PIN_YELLOW, HIGH);
  } else if (message == "LED_YELLOW_OFF") {
    digitalWrite(LED_PIN_YELLOW, LOW);
  } else if (message == "LED_RED_ON") {
    digitalWrite(LED_PIN_RED, HIGH);
  } else if (message == "LED_RED_OFF") {
    digitalWrite(LED_PIN_RED, LOW);
  }
}

// Callback function ketika menerima pesan dari MQTT
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    const char* actuatorId = doc["actuatorId"];
    const char* status = doc["status"];

    // Mengontrol Aktuator Berdasarkan Pesan
    if (String(actuatorId) == "actuator-1") {
        if (String(status) == "ON") {
            digitalWrite(22, HIGH);  // Aktuator 1 ON (Contoh, GPIO 22)
        } else {
            digitalWrite(22, LOW);   // Aktuator 1 OFF
        }
    }
    // Lakukan hal yang sama untuk actuator lainnya...
}
...
void reconnect() {
    while (!client.connected()) {
        ...
        if (client.connect("ESP32Client")) {
            ...
            client.subscribe("kelompok1subs"); // Subscribe ke topik actuator/control
        } else {
...
void publishMessage(float temperature_dht, float humidity, float temperature_bmp, float pressure, int beatAvg, int spo2, bool getaranDetected) {
  StaticJsonDocument<256> doc; // Increased size for JSON document
  doc["temperature_dht"] = temperature_dht;
  doc["humidity"] = humidity;
  doc["temperature_bmp"] = temperature_bmp;
  doc["pressure"] = pressure;
  doc["beatAvg"] = beatsPerMinute; // Use beatAvg instead of beatsPerMinute
  doc["spO2"] = spo2;
...
f
  ...
  if (client.publish(publishTopic, jsonBuffer)) {
    ...
    Serial.println("Failed to send data to AWS. Retrying..."); // Debug: Failure message
  }
}
...
void readBMP280Sensor(float &temperature_bmp, float &pressure) {
  ...
  pressure = bmp280.readPressure() / 1000.0F;
  delay(200);  // Delay for BMP280 sensor
}
...
void readMAX30102Sensor() {
  ...
  maxim_heart_rate_and_oxygen_saturation(irBuffer, bufferLength, redBuffer, &spo2, &validSPO2, &heartRate, &validHeartRate);
  delay(200);  // Delay for MAX30102 sensor
}
...
void setup() {
  ...
  particleSensor.setPulseAmplitudeIR(0x1F);  // Increase the pulse amplitude
...
void loop() {
  ...
  delay(200);  // Delay before reading sensors again
}
